...
def init_db():
    ...
    # NOVA Tabela de Configurações (Chave-Valor para textos longos, banners, etc.)
    cursor.execute("""
    ...
    # 2. Usuário Administrador Fixo (utbdenis6752 / 675201)
    ...
    config_keys = [
        ...
        ('banner_pagamento', 'assets/pix.png,assets/visa.png,assets/mastercard.png'), # Exemplo de lista de imagens (separadas por vírgula)
    ]
...
# 3. Funções de Manipulação de Dados (CRUD Produtos e Usuários) - MANTIDAS
# ... (Funções get_produtos, get_produto_por_id, get_user, add_or_update_produto, delete_produto, is_valid_login) ...
def get_produtos():
    conn = create_connection()
...
def add_or_update_produto(id, nome, preco, descricao, img_paths, video_path):
    conn = create_connection()
    ...
    cursor.execute("""
        ...
        WHERE id=?
    """, (nome, preco, descricao, img_1, img_2, img_3, img_4, video_path, id))
    if cursor.rowcount == 0:
        cursor.execute("""
            INSERT INTO produtos (id, nome, preco, descricao, img_path_1, img_path_2, img_path_3, img_path_4, video_path)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
...
# ... (Funções add_cliente, get_clientes, get_cliente_por_id, update_cliente, delete_cliente) ...
